...
class RelationalAlgebraFrozenSet(abc.RelationalAlgebraFrozenSet):
    def __init__(self, iterable=None):
        ...
        if iterable is not None:
            if isinstance(iterable, RelationalAlgebraFrozenSet):
                ...
                self._container = vx.from_arrays(x=iterable)
    ...
    def _drop_duplicates(container):
        if len(container) == 0 or len(container.columns) == 0:
            return container
        column_names = container.get_column_names(alias=True)
        sets = [container._set(column_name) for column_name in column_names]
        counts = [set.count for set in sets]
        set_names = [container.add_variable(f'set_{column_name}', set, unique=True) for column_name, set in zip(column_names, sets)]
        expression = container[f'_ordinal_values({column_names[0]}, {set_names[0]})'].astype('int64')
        product_count = 1
        for count, set_name, column_name in zip(counts[:-1], set_names[1:], column_names[1:]):
            product_count *= count
            expression = expression + container[f'_ordinal_values({column_name}, {set_name})'].astype('int64') * product_count
        container['row_id'] = expression
        index = container._index('row_id')
        unique_row_ids = container.row_id.unique()
        indices = index.map_index(unique_row_ids)
        container = container.take(indices).drop('row_id')
        return container
    ...
    @property
    def columns(self):
        return self._container.get_column_names(alias=True)
    ...
    def __eq__(self, other):
        if isinstance(other, type(self)):
            ...
            if self.is_empty() and other.is_empty():
                ...
                comp = scont.compare(ocont)
                res = all([not elem for elem in comp])
            else:
                res = False
            return res
        else:
    ...
    def itervalues(self):
        if self.is_empty():
            ...
            return iter(self._container.iterrows())
