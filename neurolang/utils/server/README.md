...
For convenience, if npm is available in your path, the frontend is built automatically via the hatchling build hook (`hatch_build.py`) when running `pip install`. Simply run:

```bash
$ pip install -e .
```

To force the Dask backend, set the `NEUROLANG_DASK` environment variable before installing:

```bash
$ NEUROLANG_DASK=1 pip install -e .
```

Once you start the tornado server (using `neuro-server` command or `python -m neurolang.utils.server.app`), the built application will then be available from [http://localhost:8888/](http://localhost:8888/).
