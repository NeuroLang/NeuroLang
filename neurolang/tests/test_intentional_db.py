...
class Solver(sdb.IntentionalDatabaseSolver, solver.GenericSolver):
    pass
...
def test_simple_set():
    ...
    s1 = C_[AbstractSet[int]](
        frozenset(C_(i) for i in range(5))
    )
    ...
    assert isinstance(res, Constant[bool]) and res.value

    fa = F_(s1, (C_(20),))
    res = ts.walk(fa)

    assert isinstance(res, Constant[bool]) and not res.value


def test_tuple_set():
    ts = Solver()

    s1 = C_[AbstractSet[Tuple[int, str]]](frozenset(
