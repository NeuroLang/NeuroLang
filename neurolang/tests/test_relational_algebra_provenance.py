...
def test_difference():
    ...
    result = RelationalAlgebraProvenanceCountingSolver().walk(op)
    assert eq_prov_isclose(result, r_expected)
...
def eq_prov_isclose(pas1, pas2):
    ...
    pas1 = testing.make_prov_set(
        pas1.value._container[list(pas1.value.columns)],
        pas1.value.columns)
    pas2 = testing.make_prov_set(
