...
def test_spatial_index_region_solver():
    ...
    # region symbols
    posterior_inferior = S_[Region]('posterior_inferior')
    anterior = S_[Region]('anterior')
    superior = S_[Region]('superior')

    st[posterior_inferior] = C_(Region((0, 0, 0), (1, 1, 1)))
    st[anterior] = C_(Region((0, 2, 0), (1, 3, 1)))
    st[superior] = C_(Region((0, 0, 2), (1, 1, 3)))
    ...
    query = Query[AbstractSet[Region]](
        x, F_(
            C_(operator.and_), (
                S_('posterior_of')(x, anterior),
                S_('inferior_of')(x, superior)
            )
        )
    )
    ...
    query = Query[AbstractSet[Tuple[Region, Region]]](
        C_((x, y)), (
            S_('posterior_of')(x, anterior) &
            S_('inferior_of')(y, x)
        )
...
def _profile_solver(solver_cls):
    ...
    reference = S_[Region]('reference')
    st[reference] = C_(Region((0, 0, 2), (1, 1, 3)))

    inferior = S_[Region]('inferior')
    st[inferior] = C_(Region((0, 0, 0), (1, 1, 1)))

    for i in range(300):
        symbol = S_[Region](f'superior_{i}')
        lb = np.random.randint(low=0, high=5, size=3) + [0, 0, 5]
        ub = lb + np.random.randint(low=1, high=10, size=3)
        st[symbol] = C_(Region(lb, ub))
