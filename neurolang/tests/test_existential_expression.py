...
def test_existential_elem_in_set():
    ...
    solver.symbol_table[nl.Symbol[set_type]('elements')] = nl.Constant[set_type](frozenset([1, 2, 3]))
...
def test_existential_greater_than():
    ...
    solver = SBS(TypedSymbolTable())
    values = range(0, 100)
    solver.symbol_table[nl.Symbol[set_type]('elements')] = nl.Constant[set_type](frozenset(values))
...
def test_existential_bound_variable():
    ...
    solver = SBS(TypedSymbolTable())
    solver.symbol_table[nl.Symbol[set_type]('elements')] = nl.Constant[set_type](frozenset(range(1, 100)))
    solver.symbol_table[nl.Symbol[set_type]('x')] = nl.Constant[set_type](frozenset([2]))
...
def test_existential_unsat_predicate_returns_empty():
    ...
    set_type = AbstractSet[int]
    solver = SBS(TypedSymbolTable())
    solver.symbol_table[nl.Symbol[set_type]('elements')] = nl.Constant[set_type](frozenset(range(1,500)))
...
def test_existential_predicates_conjuntion_and_disjunction():
    ...
    solver = SBS(TypedSymbolTable())
    solver.symbol_table[nl.Symbol[set_type]('elements')] = nl.Constant[set_type](frozenset([1, 2, 100]))
    pred_type = Callable[
        [set_type],
        set_type
    ]
    ...
    p2 = Predicate[set_type](
        Symbol('sum_less_than_100'),
        (Symbol[set_type]('x'),)
    )

    exists = ExistentialPredicate[set_type](
        Symbol[set_type]('x'), p1 & p2
    )
