...
class QueryBuilder:
    ...
    def add_region(self, region, result_symbol_name=None):
        if not isinstance(region, self.solver.type):
            raise ValueError(f"type mismatch between region and solver type: {self.solver.type}")
    ...
    def add_region_set(
        ...
        if not isinstance(region_set, Container):
            raise ValueError(f"type mismatch between region and solver set type {self.set_type}")
