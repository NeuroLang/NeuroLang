...
class FMAOntology():
    ...
    def init_ontology(self, neurolangDL, root_node='Segment of brain', destriuex_relations=False):
        ...
        if destriuex_relations:
            ...
            for label_number, name in destrieux_dataset['labels']:
                if label_number == 0:
                    continue
                name = name.decode()
                region = neurolangDL.create_region(destrieux_map, label=label_number)
                if region is None:
                    continue
            ...
            neurolangDL.add_tuple_set(
                ((e1, e2,) for e1, e2 in destrieux),
                name='relations'
            )

            neurolangDL.query(relation(x, y), neurolangDL.symbols.relations(x, y))
