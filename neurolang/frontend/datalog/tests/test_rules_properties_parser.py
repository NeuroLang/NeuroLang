...
def test_rules_properties_parsing():
    ...
    expected = {
        ...
        'arithmetic_operation': {'values': ['<operand>', '<operand> + <operand>', '<operand> - <operand>', '<operand> * <operand>', '<operand> / <operand>', '<operand> ** <operand>'], 'params': 'expandable'},
        'cmd_arg': {'values': ['<arithmetic_operation>', '<string>', '<identifier> = <arithmetic_operation>', '<identifier> = <string>', '<identifier> = <string>']},
        'cmd_identifier': {'regexp': '/\\b(?!\\bexists\\b)(?!\\b∃\\b)(?!\\bEXISTS\\b)(?!\\bst\\b)(?!\\bans\\b)[a-zA-Z_][a-zA-Z0-9_]*\\b/', 'quotes': ''},
        'command': {'values': ['.<command_identifier> (<command_arguments>)'], 'params': 'expandable'},
        'command_arguments': {'unit': '<cmd_arg>', 'sep': ',', 'params': 'number', 'min': 1},
        'command_identifier': {'values': ['<cmd_identifier>']},
        'composite_predicate': {'values': ['( <conjunction> )', '<predicate>']},
        'comparison': {'values': ['<argument> == <argument>', '<argument> < <argument>', '<argument> <= <argument>', '<argument> > <argument>', '<argument> >= <argument>', '<argument> != <argument>'], 'params': 'number'},
        'condition': {'values': ['<composite_predicate> // <composite_predicate>']},
        'conjunction': {'unit': '<predicate>', 'sep': '&', 'params': 'number'},
        'constraint': {'values': ['<conjunction> -: <identifier> ( <arguments> )'], 'params': 'expandable'},
        'double_quote_string': {'regexp': '/[^"]*/', 'quotes': '"'},
        'double_quote_text': {'regexp': '/[a-zA-Z0-9 ]*/', 'quotes': '"'},
        'existential_body': {'values': ['<arguments>; <predicates>']},
        'existential_predicate': {'values': ['exists ( <arguments>; <predicates> )']},
        'expression': {'values': ['<rule>', '<constraint>', '<fact>', '<probabilistic_rule>', '<probabilistic_fact>', '<statement>', '<statement_function>', '<command>']},
        'fact': {'values': ['<identifier> ()', '<identifier> (<fact_arguments>)'], 'params': 'expandable'},
        'fact_argument': {'values': ['<text>', '<number>', '@<identifier>']},
        'fact_arguments': {'unit': '<fact_argument>', 'params': 'number', 'sep': ','},
        'float': {'regexp': '/([0-9]+(e|E)[\\+|-][0-9]+)|(([0-9]+\\.([0-9]+)?|\\.[0-9]+)((e|E)[\\+|-][0-9]+)?)/', 'quotes': ''},
        'function': {'values': ['<function_identifier> ()', '<function_identifier> (<arguments>)'], 'params': 'expandable'},
        'function_identifier': {'values': ['<identifier>', '@<identifier>', '<lambda_expression>', '(<lambda_expression>)']},
        'head': {'values': ['<identifier> ()', '<identifier> ( <arguments> )']},
        'identifier': {'values': ['<command_identifier>', '<identifier_regexp>']},
        'identifier_regexp': {'regexp': '/[0-9a-zA-Z\\/#%\\._:-]+/', 'quotes': '`'},
        'integer': {'regexp': '/[0-9]+/', 'quotes': ''},
        'number': {'values': ['<integer>', '<float>'], 'params': 'expandable'},
        'operand': {'values': ['<function>', '( <argument> )', '<number>', '<text>', '<signed_identifier>']},
