...
from ..standard_syntax import ExternalSymbol, parser
    ...
    # def test_facts():
    #     # print("")
    #     # print("___ test_facts() ___")
    #     res = parser('A(3)')            # ok with LALR
    #     assert res == Union((Fact(Symbol('A')(Constant(3))),))
    #
    #     res = parser('A("x")')          # ok with LALR
    #     assert res == Union((Fact(Symbol('A')(Constant('x'))),))
    #
    #     res = parser("A('x', 3)")       # ok with LALR
    #     assert res == Union((Fact(Symbol('A')(Constant('x'), Constant(3))),))
    #
    #     # UnexpectedToken: Unexpected token Token('$END', '') at line 4, column 14.
    #     # Expected one of:
    #     #         *COMPARISON_OPERATOR
    #     res = parser(                   # ok with LALR
    #         'A("x", 3)\n'
    ...
    #     ))
    #
    #     res = parser('A(x):-~B(x)')               # ok with LALR
    #     assert res == Union((
    ...
    #     ))
    #
    #     res = parser('0.8::A("a b", 3)')    # ok with LALR
    #     assert res == Union((
    #         Implication(
    #             ProbabilisticFact(
    #                 Constant(0.8),
    ...
    #     x = Symbol("x")
    #     B = Symbol("B")
    #     # UnexpectedToken: Unexpected token Token('CONJUNCTION_SYMBOL', '&') at line 2, column 34.
    #     # Expected one of:
    #     #     *COMPARISON_OPERATOR
    ...
    #     C = Symbol('C')
    #     x = Symbol('x')
    #     res = parser('C(x) :- A(x) // B(x)')    # ok with LALR
    #
    #     expected = Union((
    #         Implication(
    #             C(x),
    ...
    #     assert res == expected
    #
    #     # UnexpectedToken: Unexpected token Token('COMMA', ',') at line 2, column 14.
    #     # Expected one of:
    #     #     *COMPARISON_OPERATOR
    #     #     *PLUS
    #     #     *STAR
    ...
    #
    #     assert res == expected
    #     # UnexpectedToken: Unexpected token Token('__ANON_2', '//') at line 2, column 14.
    #     # Expected one of:
    #     #     *COMPARISON_OPERATOR
    ...
    #     s2 = Symbol("s2")
    #
    #     # UnexpectedToken: Unexpected token Token('COMMA', ',') at line 2, column 13.
    #     # Expected one of:
    #     #     *COMPARISON_OPERATOR
...
def test_prob_explicit():
    ...
    res = parser("B(x, PROB(x, y), y) :- C(x, y) // B(x)")
    assert res == Union(
        (Implication(B(x, PROB(x, y), y), Conjunction((C(x, y),))),)
    )
...
#     x = Symbol("x")
#
#     res = parser("c := (lambda x: x + 1)(2)")       # ok with LALR
...
#     res = parser('.load_csv(A, "http://myweb/file.csv", B)')    # ok with LALR
#     expected = Union(
#         (
#             Command(
#                 Symbol("load_csv"),
...
#     assert res == expected
#
#     res = parser('.load_csv("http://myweb/file.csv")')      # ok with LALR
#     expected = Union(
#         (
#             Command(
...
#     assert res == expected
#
#     res = parser(".load_csv()")         # ok with LALR
#     expected = Union((Command(Symbol("load_csv"), (), ()),))
#     assert res == expected
#
#     res = parser('.load_csv(sep=",")')      # ok with LALR
#     expected = Union(
#         (Command("load_csv", (), ((Symbol("sep"), Constant(",")),)),)
#     )
#     assert res == expected
#
#     res = parser('.load_csv(sep=",", header=None, index_col=0)')    # ok with LALR
#     expected = Union(
#         (
#             Command(
#                 "load_csv",
...
#     assert res == expected
#
#     # UnexpectedToken: Unexpected token Token('EQUAL', '=') at line 2, column 42.
#     # Expected one of:
#     #     *FUNC_L_PAR
#     #     *FUNC_R_PAR
#     #     *PLUS
