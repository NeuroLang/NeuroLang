...
def test_simpsons_paradox():
    ...
    code = Union(
        (
            Implication(
                Drug(g, d), Conjunction((Gender(g), U_Recovery(g, d, r)))
            ),
            Implication(
                Recovery(g, d, r),
                Conjunction((Drug(g, d), U_Recovery(g, d, r), Gender(g))),
            ),
            Implication(
                ans(r, ProbabilisticQuery(PROB, (r,))),
                Condition(
                    Recovery(g, d, r),
                    Conjunction((CausalIntervention((Gender(men),)),)),
                ),
            ),
        )
    )
