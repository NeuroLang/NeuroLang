...
def is_conjunction(expression):
    return (
        ...
        and isinstance(expression.functor, Constant)
        and expression.functor.value == operator.and_
    )


def get_conjunction_atoms(expression):
    if is_conjunction(expression):
        return get_conjunction_atoms(
            expression.args[0]
...
def get_predicate_probabilistic_rules(gdatalog, predicate):
    ...
    return set(
        rule
        for rule in gdatalog.symbol_table[predicate].expressions
