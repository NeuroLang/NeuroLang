...
q1 = ''.join(
    'ans(name, region_1):-destrieux(name, region_1),'
...
q2 = ''.join(
    'ans(study_id):-neurosynth_default_mode_study_id(study_id),'
    'neurosynth_pcc_study_id(study_id)'
)

with nl.scope as e:
    nl.execute_nat_datalog_program(q2)
    q2_res = nl.solve_all()["ans"]
...
########################################################################
# Query ids of studies related to both terms "default mode" and
# "pcc" in the Neurosynth database. Return the study id, the term and
# the term's tf-idf score.
# ----------------------------------------------------------------------
